...
- **`EngineeringContext`**: An immutable snapshot of the workspace (files, artifacts, dependencies) at the _start_ of the planning phase.
- **`ExecutionPlan`**: An immutable blueprint generated by the planner. It defines _what_ should happen (a list of `ExecutionStep`s) but contains no runtime state.
- **`ExecutionSession`**: A mutable entity that tracks _what is currently happening_. It contains the current status (Started, Completed, Failed), retry counts, and references the active step.
