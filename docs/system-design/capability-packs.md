...
- **Primary Baseline:** `Vision`, `Requirements`, `Architecture`, `SystemDesign`. These form the linear approval cascade. They define the _what_ and the _how_.
- **Derived Artifacts:** `UserStories`, `ImplementationPlan`. These are operational artifacts generated _from_ the primary baseline. They exist alongside the main sequence and do not block downstream approval gates.
